...
for (let offset = -TOTAL_DAYS; offset <= 0; offset++) {
  ...
  for (let r = 0; r < rollsCount; r++) {
    ...
    // For today (offset === 0), only include rolls produced before the current time
    if (offset === 0 && hour >= currentHour) continue;
  ...
  // For today, set running time up to current hour (machine still running)
  const todayOffTime = offset === 0 ? `${String(Math.min(currentHour, 23)).padStart(2, '0')}:${String(new Date().getMinutes()).padStart(2, '0')}` : '23:30';
  ...
  // For today, limit reels proportionally to rolls produced so far
  const effectiveReels = (offset === 0 && dailyRolls.length > 0) ? Math.min(reelsCount, dailyRolls.length + 1) : reelsCount;
  for (let k = 0; k < effectiveReels; k++) {
    if (dailyRolls.length === 0) break; // No rolls yet today, skip reels
    ...
    INITIAL_REWINDER_REELS.push({
      ...
      date,
      runningRollNo: matchingRoll.rollNo,
      runningSize: '1650 mm',
      productName: prod.name,
