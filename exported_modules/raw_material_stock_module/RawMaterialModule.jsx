      ...
      <div className="bg-white dark:bg-[#12162B] rounded-2xl p-4 sm:p-6 border border-[#EEF0F5] dark:border-[#222943] shadow-[0_8px_24px_rgba(0,0,0,0.04)] space-y-5">
          ...
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-[#222943] font-medium">
              {filteredItems.map(item => {
                ...
                const minLimit = item.minStock || 1;
                const isLow = currentQty <= item.minStock;
                const isNegative = currentQty < 0;
                // Full bar (100%) when stock is at or above minimum limit; decreases when stock falls under limit
                const stockPercent = currentQty >= minLimit 
                  ? 100 
                  : isNegative 
                    ? 0 
                    : Math.max(Math.min((currentQty / minLimit) * 100, 100), 2);

                return (
                  <React.Fragment key={item.id}>
                    <tr className="hover:bg-slate-50/60 dark:hover:bg-[#181D35]/60 transition-colors">
                    ...
                    {/* Horizontal Stock Level Progress Bar under the data line */}
                    <tr key={`${item.id}-bar`}>
                      <td colSpan="6" className="px-3.5 pb-3.5 pt-0 border-b border-slate-100 dark:border-[#222943]">
                        <div className="w-full bg-slate-100 dark:bg-[#1C2237] h-2 rounded-full overflow-hidden p-[1px]">
                          <div 
                            className={`h-full rounded-full transition-all duration-500 ${
                              isNegative ? 'bg-[#F1533C]' : isLow ? 'bg-[#F59E0B]' : 'bg-[#cf8730]'
                            }`}
                            style={{ width: `${stockPercent}%` }}
                            title={`Stock Level: ${stockPercent.toFixed(0)}% ${currentQty >= minLimit ? '(Full Stock)' : '(Below Limit)'}`}
                          />
                        </div>
                      </td>
                    </tr>
        ...
        <div className="md:hidden space-y-3">
          {filteredItems.map(item => {
            ...
            return (
              <div key={item.id} className="p-4 rounded-2xl bg-[#F5F6FA] dark:bg-[#181D35] border border-[#EEF0F5] dark:border-[#262D4A] space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-sm font-bold text-[#161B26] dark:text-white">{item.name}</span>
                  {isNegative ? (
                ...
                <div className="grid grid-cols-2 gap-2 text-xs pt-1">
                  <div>
                    ...
                    <span className="font-semibold text-slate-700 dark:text-slate-300 capitalize">{item.category.replace('_', ' ')}</span>
                  </div>
                  <div>
                    <span className="text-slate-400 block">Current Stock</span>
                    <span className={`font-extrabold ${isNegative ? 'text-[#F1533C]' : 'text-[#161B26] dark:text-white'}`}>
                      {currentQty.toLocaleString()} {item.unit}
                    </span>
                  </div>
                </div>
                ...
                <div className="pt-2">
                  <div className="flex justify-between text-[10px] font-bold text-slate-400 mb-1">
                    <span>Stock Level Bar</span>
                    <span>{currentQty >= minLimit ? '100% (Full)' : `${stockPercent.toFixed(0)}% (Below Limit)`}</span>
                  </div>
                  <div className="w-full bg-slate-200 dark:bg-[#1C2237] h-2 rounded-full overflow-hidden p-[1px]">
                    <div 
                      className={`h-full rounded-full transition-all duration-500 ${
                        isNegative ? 'bg-[#F1533C]' : isLow ? 'bg-[#F59E0B]' : 'bg-[#cf8730]'
                      }`}
